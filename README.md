...
**극장을 추가하면 좌석 조건이 그 관에서도 통하는지 확인해야 한다.** 관마다 열
구성과 폭이 달라서, 한 극장에 맞춰 둔 `min_row` / `center_ratio` 가 다른 극장에서는
아무것도 통과시키지 못할 수 있다. 그러면 감시는 도는데 영원히 예매하지 않는다.
조용히 실패하는 종류라 눈으로 봐야 한다.

```
python tools/check_seats.py
```

극장마다 몇 회차의 좌석맵을 실제로 받아 "API 잔여석 / 진짜 구매 가능 / 조건 통과"
를 나란히 보여 준다. '구매가능'은 많은데 '조건통과'가 0이면 조건이 그 관에 안 맞는
것이고, '구매가능'부터 0이면 그냥 매진이다.
